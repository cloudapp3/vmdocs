...
const consentDict: Record<string, ConsentMessages> = {
  'en-US': {
    before:
      'We use cookies to measure traffic with Google Analytics. You can accept or reject these cookies at any time. See our ',
  ...
  'zh-CN': {
    before:
      '我们使用 Google Analytics 统计流量,因此会使用 Cookie。您可以随时接受或拒绝。详见',
    privacy: '隐私政策',
    after: '。',
    accept: '接受',
    reject: '拒绝',
  ...
  'ja-JP': {
    before:
      '当サイトは Google Analytics によるアクセス解析のため Cookie を使用します。いつでも許可・拒否できます。',
    privacy: 'プライバシーポリシー',
    after: 'をご覧ください。',
    accept: '同意する',
    reject: '拒否',
  ...
  'ru-RU': {
    before:
      'Мы используем cookie для измерения трафика с помощью Google Analytics. Вы можете в любой момент принять или отклонить их. См. ',
  ...
  'es-ES': {
    before:
      'Usamos cookies para medir el tráfico con Google Analytics. Puedes aceptar o rechazar estas cookies en cualquier momento. Consulta nuestra ',
  ...
  'pt-BR': {
    before:
      'Usamos cookies para medir o tráfego com o Google Analytics. Você pode aceitar ou rejeitar esses cookies a qualquer momento. Consulte nossa ',
  ...
  'ko-KR': {
    before:
      'Google Analytics 트래픽 측정을 위해 쿠키를 사용합니다. 언제든지 수락하거나 거부할 수 있습니다. ',
    privacy: '개인정보 처리방침',
    after: '을 참조하세요.',
    accept: '수락',
    reject: '거부',
