...
hero:
  ...
  tagline: Кроссплатформенный мониторинг рантайма хоста для Linux, macOS и Windows — через TUI, JSON-вывод, веб-дашборд или встраиваемые Go API.
  image:
    src: /logo.svg
    alt: vminfo
  actions:
...
features:
  ...
  - icon: 🌐
    ...
    details: Запускайте веб-дашборд только для чтения с REST- и WebSocket-эндпоинтами и переключаемыми темами.
...
vminfo создан для разработчиков, SRE- и DevOps-инженеров и администраторов серверов, которым нужен простой способ получить видимость состояния машины.
...
- открыть веб-дашборд на сервере через `vminfo --web`
